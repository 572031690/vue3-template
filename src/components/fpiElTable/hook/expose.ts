...
import type { dataTs } from '../types'

export function exposeHook(data: dataTs, props: Record<string, any>, singleTableRef: any, fun: {
    ...
    const getTableData = async (types?: 'all' | number | {
        ...
        if (!types || !props.isShowPage)
            return cloneDeep(data.currTableData)
        if (types === 'all') {
            const returnData = await fun.sendRequest(1, data.pageParameter.total, true)
            return returnData
        }
        if (typeof types === 'number')
            return cloneDeep(data.currTableData[types])
    ...
    const reLoadTable = async () => {
        // 判断是否转入api 有则请求数据
        if (props.api)
            data.currTableData = await fun.filterData()
    ...
    const setCurrentRow = (row?: Record<string, any> | number) => {
        if (typeof row === 'number')
            singleTableRef && singleTableRef.value?.setCurrentRow(data.currTableData[row])
    ...
    const toggleSelection = (rows?: any[] | 'all') => {
        if (rows === 'all') {
            ...
            rows.forEach((row) => {
                if (typeof row === 'number')
                    singleTableRef.value?.toggleRowSelection(data.currTableData[row])
    ...
    const toggleRowExpansion = (rowOrIndex: Record<string, any> | number, expanded: boolean) => {
        if (typeof rowOrIndex === 'number')
            return singleTableRef.value?.toggleRowExpansion(data.currTableData[rowOrIndex], expanded)
